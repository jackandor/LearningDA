...
start <- 1
cmp_df$i100_index_income.x <- (cmp_df$Close.x / cmp_df[start, 'Close.x'] - 1) * 100
cmp_df$i100_fund_income.x <- (cmp_df$Net.x /cmp_df[start, 'Net.x'] - 1) * 100
cmp_df$tj100_index_income.y <- (cmp_df$Close.y / cmp_df[start, 'Close.y'] - 1) * 100
cmp_df$tj100_fund_income.y <- (cmp_df$Net.y / cmp_df[start, 'Net.y'] - 1) * 100
